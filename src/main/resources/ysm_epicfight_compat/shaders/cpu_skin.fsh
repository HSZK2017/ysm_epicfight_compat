...
// CPU skinning fragment shader for converted YSM-EF meshes.
// Identical semantics to bone_skin.fsh (replicates Minecraft's entity shader:
// cutout/translucent alpha modes, overlay texelFetch, lightmap texelFetch,
// linear fog), only the GLSL version differs so the CPU render path runs on
// GPUs without compute-shader / SSBO support.
